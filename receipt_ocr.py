    ...
    for page in document.pages:
      for block in page.blocks:
        for paragraph in block.paragraphs:
          ...
          old_x = -1
          for word in paragraph.words:
            if base_line == -1 or word.bounding_box.vertices[0].x < old_x or not (word.bounding_box.vertices[0].y <= old_top and old_top <= word.bounding_box.vertices[3].y) and not (word.bounding_box.vertices[0].y <= old_bottom and old_bottom <= word.bounding_box.vertices[3].y):
